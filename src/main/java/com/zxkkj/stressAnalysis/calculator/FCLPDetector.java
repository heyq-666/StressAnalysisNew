...
public class FCLPDetector {
     ...
     * 可视化FCLP段
     */
    public void visualizeFCLPSegments(List<RRIntervalPoint> heartRatePoints) {
    ...
    class FCLPPanel extends JPanel {
        private final List<RRIntervalPoint> heartRatePoints;

        public FCLPPanel(List<RRIntervalPoint> heartRatePoints) {
        ...
        protected void paintComponent(Graphics g) {
            ...
            double minHR = heartRatePoints.stream()
                    .mapToDouble(RRIntervalPoint::getHeartRate)
                    .min().orElse(0);
            ...
            int minPosition = heartRatePoints.stream()
                    .mapToInt(RRIntervalPoint::getPosition)
                    .min().orElse(0);
            ...
            for (RRIntervalPoint point : heartRatePoints) {
            ...
            for (FCLPSegment segment : fclpSegments) {
                if (segment.getStartIndex() < heartRatePoints.size() &&
                    ...
                    RRIntervalPoint startPoint = heartRatePoints.get(segment.getStartIndex());
                    RRIntervalPoint endPoint = heartRatePoints.get(segment.getEndIndex());
